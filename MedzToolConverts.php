...
if($_POST && ($to = $_POST['to']) && ($file = $_FILES['file'])) {
	if($file['type'] == 'application/x-zip-compressed' || $file['type'] == 'application/octet-stream') {
		if($to == 2) {
			$toCharset = 'gbk';
		} else {
			$toCharset = 'utf-8';
		...
		$zip = new Zip();
		$zip->init();
		$data = $zip->extract($file['tmp_name']);
		foreach($data as $v) {
			if(preg_match('/^(.*?)\.(php|html|htm|css|js|sql|xml|json)$/i', $v['filename'])) {
				$v['data'] = array_iconv($v['data'], $toCharset);
				$match = '/^(.*?)\.xml$/i';
				if(preg_match($match, $v['filename'])) {
					$v['data'] = preg_replace(
						array(
							'/encoding\=(\"|\')(.*?)(\"|\')/i',
							'/\<charset\>(.*?)\<\/charset\>/i' // 对pw插件&模板
						),
						array(
							'encoding="' . $toCharset . '"',
							'<charset>' . $toCharset . '</charset>' // 对pw插件&模板
						),
						$v['data']
					);
				}
			}
			$zip->addFile($v['data'], $v['filename'], $v['timestamp']);
		}
		header("Content-type: application/x-zip-compressed");
		header("Content-disposition: attachment; filename=" . $file['name']);
		echo $zip->getCompressedFile();
		exit;
	}
}
 ...
 */
function array_iconv($data, $output) {
...
$style = 'html,body {margin: 0;padding: 0;width: 100%;height: 100%;background-color: #333;}';
$style .= 'html,body,div {display: block;}';
$style .= 'body {color: #333;font: 12px/1.5 Tahoma;text-align: center;}';
$style .= '.main {position: relative;display: inline-block;overflow: hidden;cursor: default;padding: 0;margin: 10px 0px;-moz-box-shadow: 0px 0px 5px #ddd;-webkit-box-shadow: 0px 0px 5px #ddd;box-shadow: 0px 0px 5px #ddd;-moz-border-radius: 5px;-webkit-border-radius: 5px;border-radius: 5px;}';
$style .= '.filename {float: left;display: inline-block;outline: 0 none;height: 32px;width: 180px;margin: 0;padding: 5px 10px;overflow: hidden;cursor: default;border: 1px solid;border-right: 0;font: 9pt/100% Arial, Helvetica, sans-serif;color: #777;text-shadow: 1px 1px 0px #fff;text-overflow: ellipsis;white-space: nowrap;-moz-border-radius: 5px 0px 0px 5px;-webkit-border-radius: 5px 0px 0px 5px;border-radius: 5px 0px 0px 5px;background: #f5f5f5;background: -moz-linear-gradient(top, #fafafa 0%, #eee 100%);background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #fafafa), color-stop(100%, #f5f5f5));filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#fafafa", endColorstr="#f5f5f5", GradientType=0);border-color: #ccc;-moz-box-shadow: 0px 0px 1px #fff inset;-webkit-box-shadow: 0px 0px 1px #fff inset;box-shadow: 0px 0px 1px #fff inset;-moz-box-sizing: border-box;-webkit-box-sizing: border-box;box-sizing: border-box;}';
$style .= '.button:hover {background: #eee;background: -moz-linear-gradient(top, #dddddd 0%, #eeeeee 100%);background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #dddddd), color-stop(100%, #eeeeee));filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#dddddd", endColorstr="#eeeeee", GradientType=0);}';
$style .= '.button {float: left;/* height: 32px; */display: inline-block;outline: 0 none;padding: 9px 12px;margin: 0;cursor: pointer;border: 1px solid;font: bold 9pt/100% Arial, Helvetica, sans-serif;-moz-border-radius: 0px 5px 5px 0px;-webkit-border-radius: 0px 5px 5px 0px;border-radius: 0px 5px 5px 0px;-moz-box-shadow: 0px 0px 1px #fff inset;-webkit-box-shadow: 0px 0px 1px #fff inset;box-shadow: 0px 0px 1px #fff inset;}';
$style .= '.button {color: #555;text-shadow: 1px 1px 0px #fff;background: #ddd;background: -moz-linear-gradient(top, #eeeeee 0%, #dddddd 100%);background: -webkit-gradient(linear, left top, left bottom, color-stop(0%, #eeeeee), color-stop(100%, #dddddd));filter: progid:DXImageTransform.Microsoft.gradient(startColorstr="#eeeeee", endColorstr="#dddddd", GradientType=0);border-color: #ccc;}';
$style .= 'select {float: left;height: 32px;width: 104px;}';
echo '<!DOCTYPE html>';
echo '<html lang="zh">';
	echo '<head>';
		echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8">';
		echo '<title>PHPWind插件以及模板编码转换！</title>';
		echo '<style type="text/css">'; echo $style; echo '</style>';
	echo '</head>';
	echo '<body>';
		echo '<div class="main">';
			echo '<form action="#" method="post" enctype="multipart/form-data">';
				echo '<input class="filename" name="file" type="file">';
				echo '<select name="to">';
					echo '<option value="1">To UTF-8</option>';
					echo '<option value="2">To GBK</option>';
				echo '</select>';
				echo '<button class="button" type="submit">转换</button>';
			echo '</form>';
		echo '</div>';
	echo '</body>';
echo '</html>';
